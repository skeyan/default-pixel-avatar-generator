...
def rainbow_background(im, W, H, pixel, palette):
    draw = ImageDraw.Draw(im)
    num_color_ranges = 4
    height_per_range = H / num_color_ranges
    pixels_per_range = int(H / pixel / num_color_ranges)

    for i in range(0, int(W / pixel)):
        for j in range(0, int(H / pixel)):
            if j < pixels_per_range:
                random_color = colors[palette][random.randint(0, min(1, len(colors[palette]) - 1))]
            elif j < pixels_per_range * 2:
                random_color = colors[palette][random.randint(min(1, len(colors[palette]) - 1), min(3, len(colors[palette]) - 1))]
            elif j < pixels_per_range * 3:
                random_color = colors[palette][random.randint(min(3, len(colors[palette]) - 1), min(5, len(colors[palette]) - 1))]
            else:
                random_color = colors[palette][random.randint(min(5, len(colors[palette]) - 1), len(colors[palette]) - 1)]
...
def draw_text(im, msg, fontsize, palette, W, H):
    ...
    try:
        ...
        print("Warning: Font 'Helvetica-Bold.ttf' not found. Using default font.")
